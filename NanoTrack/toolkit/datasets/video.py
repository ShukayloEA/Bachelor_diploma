...
class Video(object):
    def __init__(self, name, root, video_dir, init_rect, img_names,
        ...
        self.attr = attr
        self.pred_trajs = {}
        self.img_names = [x for x in img_names]
        self.imgs = None
    ...
    def __iter__(self):
        for i in range(len(self.img_names)):
            ...
            elif i >= len(self.gt_traj) or self.gt_traj[i][0] != i+1:
                yield (self.imgs[i] if self.imgs is not None else cv2.imread(self.img_names[i])), [0, 0, 0, 0, 0, 0, 0]
            else:
                yield (self.imgs[i] if self.imgs is not None else cv2.imread(self.img_names[i])), self.gt_traj[i]
